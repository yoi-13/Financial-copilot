/**
 * Database verification & admin script.
 * Run: npx tsx scripts/migrate.ts [verify|sql <query>]
 */
import { Pool } from 'pg';
...
async function run() {
  ...
  if (args[0] === 'verify') {
    const tables = ['inventory_items', 'expenses', 'daily_sales', 'daily_reports'];
    ...
    console.log('Usage: npx tsx scripts/migrate.ts [verify|sql <query>]');
  }
